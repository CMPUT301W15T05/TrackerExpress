...
public class ClaimListTest extends TestCase {
	...
	public void testRemoveClaimList() {
		...
		assertTrue("Test claim not contained", 
					testClaim.equals(claimList.getClaim(testClaim.getUuid())));
		claimList.deleteClaim(testClaim.getUuid());
		assertTrue("List size isn't small enough", claimList.size()==0);
		try{
			claimList.getClaim(testUuid);
			assertTrue("This is not supposed to have happened", false);
		} catch ( IndexOutOfBoundsException e ){
			assertTrue("This is supposed to happen", true);
		}
	}
	...
	public void testClaimOrder() {
		...
		assertTrue("List size isn't correct", claimList.size()==3);
		assertTrue("Claim '1' is not first", claimList.getClaimList().get(0).getClaimName().equals("1") );
		assertTrue("Claim '2' is not second", claimList.getClaimList().get(1).getClaimName().equals("2") );
		assertTrue("Claim '3' is not last", claimList.getClaimList().get(2).getClaimName().equals("3") );
